...
void task_3(ofstream &fout, InstructionSequence *instr_seq) {
  ...
  if (answer != "error") {
    for (Node *temp = head.next; temp != NULL;) {
  ...
  /*
      ...
      Description:
          Implement a function that prints the top values of the stack when
     “top” operation is called after the sequence of “push” or “pop” operations.
     If the “top” operation is called for an empty stack, print “-1”, If “pop”
     operation is called for an empty stack, print “error” and the program
     terminates. The stack can contain positive integers.
     ...
     will print nothing)
          - (‘top’,NULL): print the top value of the current stack (print ‘-1’
  ...
  */
  void task_4(ofstream & fout, InstructionSequence * instr_seq) {
    ...
    for (int i = 0; i < instr_seq->length; i++) {
      ...
      if (command.compare("push") == 0) {
        ...
        if (instr_seq->instructions[i].value <= 0) {
          ...
          Node *temp = new Node;
          temp->value = instr_seq->instructions[i].value;
          temp->next = NULL;
        ...
        if (is_empty) {
          ...
          answer += to_string(head.top->value) + " ";
        }
      } else {
        cerr << "Invalid command" << endl;
        exit(-1);
    ...
    for (Node *temp = head.top; temp != NULL;) {
      Node *del = temp;
      temp = temp->next;
      delete del;
    }
  ...
  */
  void task_5(ofstream & fout, InstructionSequence * instr_seq) {
    ...
    for (int i = 0; i < instr_seq->length; i++) {
      ...
      if (command.compare("enqueue") == 0) {
        ...
        if (cnt >= 4) {
          ...
          queue[end % 4] = instr_seq->instructions[i].value;
          is_empty = false;
          cnt++;
          end++;
        }
    ...
    if (answer != "error") {
      ...
      for (int j = 0; j < cnt; j++) {
        answer = answer + to_string(queue[start]) + " ";
        start++;
        if (start > 3) {
          start %= 4;
        }
    ...
    for (int i = 0; i < instr_seq->length; i++) {
      ...
      if (command.compare("insert_front") == 0) {
        ...
        if (cnt == 4) { // full
          ...
          answer = to_string(instr_seq->instructions[i].value) + " " + answer;
          cnt++;
        }
        ...
        if (cnt == 4) { // full
          ...
          answer = answer + to_string(instr_seq->instructions[i].value) + " ";
          cnt++;
        }
